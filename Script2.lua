...
-- Function to click dungeon buttons sequence (FASTER TIMINGS)
local function ClickDungeonButtons()
    ...
    -- Create button (reduced from 2 to 0.5 seconds)
    task.wait(0.5)
    ...
    local createButton = player:WaitForChild("PlayerGui")
        ...
        :WaitForChild("Dungeon")
        :WaitForChild("Create")
        :FindFirstChild("Create", 1) -- Reduced from 3 to 1 second
    ...
    -- Join/Start button (reduced from 2 to 0.5 seconds)
    task.wait(0.5)
    ...
    local joinButton = player:WaitForChild("PlayerGui")
        ...
        :WaitForChild("Start", 1) -- Reduced from 3 to 1 second
...
local function createUI()
        ...
        dungeonConnection = RunService.Heartbeat:Connect(function()
            ...
            if CurrentRod then
                ...
                if success then
                    ...
                    task.wait(1) -- Reduced from 2 seconds
                end
            else
    ...
    -- Auto Dungeon Buttons System (FASTER)
    ...
    -- Auto-start functions based on saved state (PERSISTENT BUTTONS)
    if movementState.autoTeleportEnabled then
        task.wait(1)
        StartAutoTeleport()
    end
...
-- Handle character respawns (PERSISTENT BUTTONS - functions auto-restart)
player.CharacterAdded:Connect(function(character)
    ...
    -- PERSISTENT: Restart functions based on saved state
    if movementState.isMoving then
