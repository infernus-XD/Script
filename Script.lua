...
-- Function to click dungeon buttons sequence (FASTER TIMINGS - 1-2 seconds max)
local function ClickDungeonButtons()
    -- Wait for UI to load (reduced from 3 to 1 second)
    task.wait(1)
    ...
    local gemsButton = player:WaitForChild("PlayerGui")
        ...
        :WaitForChild("Gems", 2) -- Reduced from 5 to 2 seconds
    ...
    -- Create button (reduced from 2 to 1 second)
    task.wait(1)
    ...
    local createButton = player:WaitForChild("PlayerGui")
        ...
        :WaitForChild("Dungeon")
        :WaitForChild("Create")
        :FindFirstChild("Create", 2) -- Reduced from 3 to 2 seconds
    ...
    -- Join/Start button (reduced from 2 to 1 second)
    task.wait(1)
    ...
    local joinButton = player:WaitForChild("PlayerGui")
        ...
        :WaitForChild("Start", 2) -- Reduced from 3 to 2 seconds
...
local function createUI()
    ...
    local function TeleportToDungeonRod()
        -- FIXED: Only teleport if CurrentRod actually exists
        ...
        teleportConnection = RunService.Heartbeat:Connect(function()
            ...
            if Target and IsMobAlive(Target) then
                ...
                if Distance > 10 then
                    ...
                    task.wait(1) -- Reduced from longer waits
                else
                    task.wait(0.5)
                end
            else
    ...
    -- Auto Dungeon System (FIXED: Only teleports when rod exists)
    local function StartAutoDungeon()
        ...
        dungeonConnection = RunService.Heartbeat:Connect(function()
            ...
            -- FIXED: Only teleport if dungeon rod actually exists
            if CurrentRod then
                local success = TeleportToDungeonRod()
                if success then
                    dungeonButton.Text = "DUNGEON: AT PORTAL"
                    task.wait(2) -- Reduced from 5 seconds
                else
                ...
                DetectDungeonRod() -- Check if rod exists now
                task.wait(2) -- Reduced from 3 seconds
            end
        end)
    end
    ...
    -- Auto Dungeon Buttons System (FASTER: Reduced wait times)
    local function StartAutoDungeonButtons()
        ...
        dungeonButtonsConnection = RunService.Heartbeat:Connect(function()
            ...
            if CurrentRod then
                ClickDungeonButtons()
                task.wait(5) -- Reduced from 10 seconds
            else
                task.wait(2) -- Reduced from 5 seconds
            end
        end)
...
-- Handle character respawns (FIXED: Buttons stay on after respawn)
