...
local function createUI()
    ...
    frame.Size = UDim2.new(0, 200, 0, 240) -- Increased height for new button
    ...
    -- Create Preset Toggle Button
    local presetButton = Instance.new("TextButton")
    presetButton.Size = UDim2.new(0.9, 0, 0, 25)
    presetButton.Position = UDim2.new(0.05, 0, 0, 125)
    presetButton.BackgroundColor3 = movementState.presetEnabled and Color3.fromRGB(50, 150, 50) or Color3.fromRGB(180, 180, 50)
    presetButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    presetButton.Text = movementState.presetEnabled and "PRESET: ON" or "PRESET: OFF"
    presetButton.Font = Enum.Font.GothamBold
    presetButton.TextSize = 10
    presetButton.Parent = contentFrame

    -- Create Start button
    local startButton = Instance.new("TextButton")
    startButton.Size = UDim2.new(0.9, 0, 0, 25)
    startButton.Position = UDim2.new(0.05, 0, 0, 155)
    ...
    stopButton.Position = UDim2.new(0.05, 0, 0, 185)
    ...
    for _, button in pairs({startButton, stopButton, shiftLockButton, teleportButton, dungeonButton, dungeonButtonsButton, presetButton}) do
    ...
    -- Preset Functions
    local function togglePreset()
        if movementState.presetEnabled then
            -- Turn off preset and all associated features
            movementState.presetEnabled = false
            presetButton.BackgroundColor3 = Color3.fromRGB(180, 180, 50)
            presetButton.Text = "PRESET: OFF"
            ...
            if movementState.autoDungeonButtonsEnabled then
                toggleAutoDungeonButtons()
            end
        else
            -- Turn on preset and all associated features
            movementState.presetEnabled = true
            presetButton.BackgroundColor3 = Color3.fromRGB(50, 150, 50)
            presetButton.Text = "PRESET: ON"
            
            -- Turn on all auto features
    ...
    local function StopAutoTeleport()
        ...
        -- Update preset state if any auto feature is turned off
        if movementState.presetEnabled and (not movementState.autoTeleportEnabled or not movementState.autoDungeonEnabled or not movementState.autoDungeonButtonsEnabled) then
            movementState.presetEnabled = false
            presetButton.BackgroundColor3 = Color3.fromRGB(180, 180, 50)
            presetButton.Text = "PRESET: OFF"
        end
    ...
    presetButton.MouseButton1Click:Connect(togglePreset)
...
player.CharacterAdded:Connect(function(character)
    ...
    -- Restart functions based on saved state
