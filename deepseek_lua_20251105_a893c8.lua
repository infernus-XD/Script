...
-- Store UI state to persist across respawns - SET DEFAULTS TO ON
local movementState = {
    isMoving = false,
    shiftLockEnabled = false,
    autoTeleportEnabled = true,  -- DEFAULT ON
    autoDungeonEnabled = true,   -- DEFAULT ON
    autoDungeonButtonsEnabled = true,  -- DEFAULT ON
    isMinimized = false
}
...
local function createUI()
    ...
    -- Create Auto Teleport to Mob Toggle (DEFAULT ON)
    ...
    -- Create Auto Dungeon Teleport Toggle (DEFAULT ON)
    ...
    -- Create Auto Dungeon Buttons Toggle (DEFAULT ON)
    ...
    -- Auto-start functions based on saved state - ALL THREE DEFAULT TO ON
    task.wait(1)
    StartAutoTeleport()
    StartAutoDungeon()
    StartAutoDungeonButtons()
...
-- Handle character respawns - AUTO RESTART ALL THREE FEATURES
player.CharacterAdded:Connect(function(character)
    ...
    -- AUTO RESTART ALL THREE FEATURES (since they default to ON)
    task.wait(1)
