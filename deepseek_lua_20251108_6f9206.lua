...
-- PERSISTENT STATE (will survive respawns)
local movementState = {
    isMoving = false,
...
-- Game variables (will be reconnected on respawn)
local Mobs, DungeonFolder, CurrentRod
...
-- SAFE Function to reconnect game objects after respawn
local function ReconnectGameObjects()
    local success, err = pcall(function()
        -- Safely find Mobs folder
        local main = workspace:WaitForChild("__Main")
        if main then
            Mobs = main:WaitForChild("__Enemies")
            DungeonFolder = main:WaitForChild("__Dungeon")
            DetectDungeonRod()
            print("[Movement Control] Game objects reconnected after respawn")
        else
            warn("[Movement Control] __Main not found")
        end
    end)
    
    if not success then
        warn("[Movement Control] Failed to reconnect game objects: " .. tostring(err))
        -- Retry after 2 seconds
        task.delay(2, ReconnectGameObjects)
    end
end

-- Update character reference and reconnect objects on respawn
player.CharacterAdded:Connect(function(newChar) 
    Character = newChar 
    task.wait(1) -- Wait for character to fully load
    
    -- Safe reconnect
    ReconnectGameObjects()
    
    -- RESTART FEATURES BASED ON PERSISTENT STATE
    task.wait(0.5) -- Small delay to ensure objects are connected
    
    if movementState.isMoving then
        task.spawn(startMovement)
    end
    if movementState.autoTeleportEnabled then
        task.spawn(StartAutoTeleport)
    end
    if movementState.autoDungeonEnabled then
        task.spawn(StartAutoDungeon)
    end
    if movementState.autoDungeonButtonsEnabled then
        task.spawn(StartAutoDungeonButtons)
    end
    print("[Movement Control] Features restored after respawn")
end)
    ...
    local success, result = pcall(function()
        ...
        if rod then
            ...
            if primary then
                ...
                if CurrentRod then
                    print("[Movement Control] Detected DungeonRod")
                    return true
                end
            end
        end
...
-- SAFE Monitor for dungeon rod spawns
if DungeonFolder then
    ...
    DungeonFolder.DescendantRemoving:Connect(function(obj)
        if obj.Name == "DungeonRod" then
            CurrentRod = nil
            print("[Movement Control] DungeonRod removed")
        end
    end)
end
...
-- SAFE Function to click dungeon buttons sequence
local function ClickDungeonButtons()
    ...
    local gemsButton
    pcall(function()
        gemsButton = playerGui:WaitForChild("Menus"):WaitForChild("Dungeon"):WaitForChild("Create"):WaitForChild("Completed"):WaitForChild("Gems", 2)
    end)
    ...
    local createButton
    pcall(function()
        createButton = playerGui:WaitForChild("Menus"):WaitForChild("Dungeon"):WaitForChild("Create"):FindFirstChild("Create", 1)
    end)
    ...
    local joinButton
    pcall(function()
        joinButton = playerGui:WaitForChild("Menus"):WaitForChild("Dungeon"):WaitForChild("InDungeon"):WaitForChild("Start", 1)
    end)
    ...
    if joinButton then
        ...
        movementState.joinButtonClicked = true
        print("[Movement Control] Join button clicked - will reactivate features in 5 seconds")
        
        -- Start countdown to reactivate features
        task.spawn(function()
            task.wait(5)
            if movementState.joinButtonClicked then
                movementState.joinButtonClicked = false
                ReactivateDungeonFeatures()
            end
        end)
    end
end

-- Function to reactivate dungeon features after join button is clicked
local function ReactivateDungeonFeatures()
    print("[Movement Control] Reactivating dungeon features...")
    
    -- Reactivate auto teleport if it was enabled
    if movementState.autoTeleportEnabled then
        task.spawn(function()
            task.wait(0.5)
            StartAutoTeleport()
    ...
    -- Reactivate auto dungeon if it was enabled
    if movementState.autoDungeonEnabled then
        task.spawn(function()
            task.wait(1)
            StartAutoDungeon()
    ...
    -- Reactivate auto dungeon buttons if it was enabled
    if movementState.autoDungeonButtonsEnabled then
        task.spawn(function()
            task.wait(1.5)
...
local function createUI()
    ...
    -- Create ScreenGui WITH PERSISTENCE
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "MovementControl"
    screenGui.ResetOnSpawn = false  -- KEY: This prevents UI from resetting on respawn
    ...
    madeByLabel.Position = UDim2.new(0, 0, 1, -15)
    madeByLabel.BackgroundTransparency = 1
    madeByLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
    madeByLabel.Text = "MADE BY - DADDYFERNUS"
    ...
    -- Create Shift Lock Toggle (with persistent state)
    ...
    -- Create Auto Teleport to Mob Toggle (with persistent state)
    ...
    -- Create Auto Dungeon Teleport Toggle (with persistent state)
    ...
    -- Create Auto Dungeon Buttons Toggle (with persistent state)
    ...
    function StartAutoDungeon()
        ...
        if Connections.dungeon then
            Connections.dungeon:Disconnect()
        end
        
        Connections.dungeon = RunService.Heartbeat:Connect(function()
            if not movementState.autoDungeonEnabled then 
                if Connections.dungeon then
                    Connections.dungeon:Disconnect()
                    Connections.dungeon = nil
                end
                return 
            end
    ...
    function StartAutoDungeonButtons()
        ...
        if Connections.dungeonButtons then
            Connections.dungeonButtons:Disconnect()
        end
        
        Connections.dungeonButtons = RunService.Heartbeat:Connect(function()
            if not movementState.autoDungeonButtonsEnabled then 
                if Connections.dungeonButtons then
                    Connections.dungeonButtons:Disconnect()
                    Connections.dungeonButtons = nil
                end
                return 
            end
    ...
    function StopAutoDungeonButtons()
...
-- Handle character respawns (with persistence)
player.CharacterAdded:Connect(function(character)
    ...
    -- Restart functions based on PERSISTENT STATE
    if movementState.isMoving then
        task.spawn(uiControls.startMovement)
    end
    
    if movementState.autoTeleportEnabled then
        task.spawn(uiControls.StartAutoTeleport)
    end
    
    if movementState.autoDungeonEnabled then
        task.spawn(uiControls.StartAutoDungeon)
    end
    
    if movementState.autoDungeonButtonsEnabled then
        task.spawn(uiControls.StartAutoDungeonButtons)
    end
    
    -- Recreate UI if it was destroyed (but with ResetOnSpawn = false, it shouldn't be)
    local playerGui = player:FindFirstChild("PlayerGui")
...
print("=== MOVEMENT CONTROL PERSISTENT LOADED ===")
print("Buttons will stay ON after character respawn!")