...
local function createUI()
    ...
    -- Restart functions based on saved state immediately after UI creation
    if movementState.autoTeleportEnabled then
...
-- Handle character respawns - IMPROVED PERSISTENCE
player.CharacterAdded:Connect(function(character)
    ...
    print("[Movement Control] Character respawned - restoring state...")
    
    -- Recreate UI to ensure fresh connections
    local playerGui = player:FindFirstChild("PlayerGui")
    if playerGui and playerGui:FindFirstChild("MovementControl") then
        playerGui.MovementControl:Destroy()
    end
    ...
    -- The UI recreation now automatically restarts all functions based on movementState
end)

player.CharacterRemoving:Connect(function()
    -- We don't stop functions here anymore - let them persist through respawn
    -- The functions will automatically use the new character when it's available
    print("[Movement Control] Character removed - keeping functions active")
end)

-- UI Persistence System (removed the aggressive recreation loop)
-- The UI will now only recreate on character respawn or manual closure