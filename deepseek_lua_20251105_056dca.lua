...
-- Store UI state to persist across respawns - USE GLOBAL TABLE FOR PERSISTENCE
_G.MovementControlState = _G.MovementControlState or {
    isMoving = false,
...
local movementState = _G.MovementControlState
...
player.CharacterAdded:Connect(function(character)
    ...
    print("[Movement Control] Character respawned, restoring states...")
    
    -- Restart functions based on saved state - WITH BETTER ERROR HANDLING
    if movementState.isMoving then
        pcall(function()
            uiControls.startMovement()
        end)
        print("[Movement Control] Movement restored:", movementState.isMoving)
    end
    ...
    if movementState.autoTeleportEnabled then
        ...
        print("[Movement Control] Auto Teleport restored:", movementState.autoTeleportEnabled)
    end
    ...
    if movementState.autoDungeonEnabled then
        ...
        print("[Movement Control] Auto Dungeon restored:", movementState.autoDungeonEnabled)
    end
    ...
    if movementState.autoDungeonButtonsEnabled then
        ...
        print("[Movement Control] Auto Dungeon Buttons restored:", movementState.autoDungeonButtonsEnabled)
    end
    ...
    if not playerGui or not playerGui:FindFirstChild("MovementControl") then
        task.wait(1)
        print("[Movement Control] Recreating UI after respawn...")
        uiControls = createUI()
    end
end)

-- DON'T STOP THE FUNCTIONS WHEN CHARACTER IS REMOVED - JUST DISCONNECT THE CONNECTIONS
-- This allows the state to persist and automatically restart when new character loads
player.CharacterRemoving:Connect(function()
    -- Only disconnect connections but keep the state enabled
    if uiControls then
        -- Store current states before disconnecting
        local wasMoving = movementState.isMoving
        local wasTeleporting = movementState.autoTeleportEnabled
        local wasDungeon = movementState.autoDungeonEnabled
        local wasDungeonButtons = movementState.autoDungeonButtonsEnabled
        
        -- Disconnect connections but don't change the state
        if movementState.isMoving then
            uiControls.stopMovement()
            movementState.isMoving = wasMoving -- Restore the state
        end
        
        if movementState.autoTeleportEnabled then
            uiControls.StopAutoTeleport()
            movementState.autoTeleportEnabled = wasTeleporting -- Restore the state
        end
        
        if movementState.autoDungeonEnabled then
            uiControls.StopAutoDungeon()
            movementState.autoDungeonEnabled = wasDungeon -- Restore the state
        end
        
        if movementState.autoDungeonButtonsEnabled then
            uiControls.StopAutoDungeonButtons()
            movementState.autoDungeonButtonsEnabled = wasDungeonButtons -- Restore the state
        end
    end
end)
...
while true do
    ...
    local playerGui = player:FindFirstChild("PlayerGui")
    if not playerGui or not playerGui:FindFirstChild("MovementControl") then
        print("[Movement Control] UI not found, recreating...")
        uiControls = createUI()
    end
end